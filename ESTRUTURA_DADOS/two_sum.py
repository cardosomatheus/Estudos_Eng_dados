...
class Solution:
    ...
    def twoSum2(self, nums: list[int], target: int) -> list[int]:
        for pos, valor in enumerate(nums):
            (target - valor)
            if (target - valor) in (nums):
                
                for pos_interna, valor_interno in enumerate(nums):
                    if pos == pos_interna:
                        continue

                    if (target - valor) == valor_interno:
                        return[pos, pos_interna]
...
inicio =datetime.now()
solucao.twoSum(nums=nums, target=alvo)
fim = datetime.now()
print(f'heashmap: {fim - inicio}')


inicio =datetime.now()
solucao.twoSum2(nums=nums, target=alvo)
fim = datetime.now()
